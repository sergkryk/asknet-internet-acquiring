...
interface FrontendVgroupsResponseWithService extends FrontendVgroupsResponse {
  service: {
    rentperiod: number;
  };
}
...
const filterAgreementFields = (
  candidate: SoapAgreement
): FrontedAgreementResponse => {
  const { agrmid, vgroups, balance, credit, number, date, operid } = candidate;
  return { agrmid, vgroups, balance, credit, number, date, operid };
};
// Filter to send only necessary object keys
function filterVgroupsFields(
  candidate: SoapClientVgroupFull[]
): FrontendVgroupsResponse[] {
...
function filterAccountFields(candidate: SoapAccount): FrontendAccountResponse {
  ...
  return {
    ...
    pass,
    isPassport:
      passissuedate && passissuedep && passissueplace && passno && passsernum
        ? true
        : false,
  };
}
...
function validateCredentials(req: Request, res: Response): Response | void {
  ...
  if (login.length < 4)
    throw new HttpError("Login must be at least 4 characters long", 400);
  if (password.length < 6)
    throw new HttpError("Password must be at least 6 characters long", 400);
  if (!/^[a-zA-Z0-9]+$/.test(password))
    throw new HttpError("Password can only contain letters and numbers", 400);
}
...
// GET controller
export const clientGetController = async function (
  req: Request,
  res: Response
) {
  try {
...
// POST controller
export const clientPostController = async function (
  req: Request,
  res: Response
) {
  try {
