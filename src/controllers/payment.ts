...
import { BankRequestCandidate, IPaymentRequestBody } from '../services/tpayments/types';
...
function isPaymentBodyValid(body: any): body is IPaymentBody {
	return (
		...
		(typeof body.email === 'string' && body.Amount >= 100 && body.Amount <= 10000)
	);
}
...
// POST controller
export const paymentController = async function (req: Request, res: Response, next: NextFunction) {
	try {
		// Validate the payment request body
